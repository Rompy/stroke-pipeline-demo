...
pipeline_x_spacing = 2.0  # Significantly increased from 1.35
output_x_start = pipeline_x_start + len(pipeline_stages) * pipeline_x_spacing + 2.0  # More gap
...
fig_flow.add_trace(go.Scatter(
    x=[input_x], y=[y_pos],
    mode='markers+text',
    marker=dict(size=115, color=input_color, line=dict(width=3, color='white')),
...
for i, stage in enumerate(output_stages):
    ...
    fig_flow.add_trace(go.Scatter(
        x=[x], y=[y_pos],
        mode='markers+text',
        marker=dict(size=115, color=output_colors[i], line=dict(width=3, color='white')),
        text=stage.replace('\n', '<br>'),
        textposition='middle center',
        textfont=dict(color='white', size=11, family='Arial Black'),
        hoverinfo='text',
...
2. 🔄 **The Pipeline** → Extract, validate, and correct data through 7 processing layers
